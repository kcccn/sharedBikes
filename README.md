...
### ✅ Phase 0 — 项目骨架与分层架构（已完成）
搭建了后端分层架构（API → Services → Core → Utils），定义了核心领域模型（City、Fleet、Weather、Scheduler、Engine），完成了基础 API 端点与单元测试框架。

### 🔜 Phase 1 — 基础设施（进行中）
**目标：** 接入真实地图数据（OpenStreetMap），完成路网解析与静态车辆投放。

| 步骤 | 说明 | 状态 |
|------|------|------|
| [#24](https://github.com/kcccn/sharedBikes/issues/24) RFC: 地图数据导入方案设计 | 讨论 OSM 解析技术选型、数据模型映射 | 🔍 待讨论 |
| [#26](https://github.com/kcccn/sharedBikes/issues/26) OSM 路网解析器 | 实现 Node/Edge 解析管线 | 📝 待实现 |
| [#28](https://github.com/kcccn/sharedBikes/issues/28) Station 自动生成算法 | 基于路网自动推荐停车点 | 📝 待实现 |
| [#29](https://github.com/kcccn/sharedBikes/issues/29) MapService 集成与测试 | 端到端管线串联 | 📝 待实现 |

### 🔮 Phase 2 — 动态城市
引入基于时间的 NPC 需求生成机制（通勤潮汐模拟）。

### 🔮 Phase 3 — 调度博弈
加入调度员派遣与财务结算系统。

### 🔮 Phase 4 — 视觉盛宴
接入 Deck.gl，完成热力图与 OD 轨迹流的高帧率渲染。
