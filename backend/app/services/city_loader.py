...
Uses Python 3.11+ built-in ``tomllib`` with a ``tomli`` fallback for
older runtimes (not that we need it, but defensive).
"""
...
class CityLoader:
    ...
    def list_available_cities(self) -> list[str]:
        ...
        if not self._config_dir.is_dir():
            return []
        return sorted(
            f.stem for f in self._config_dir.glob("*.toml")
        )
    ...
    def _parse_toml(self, raw: bytes, city_id: str) -> CityConfig:
        ...
        return CityConfig(
            ...
            default_station_capacity=city.get(
                "default_station_capacity", 30
            ),
            initial_bikes_per_station=city.get(
                "initial_bikes_per_station", 10
            ),
            ...
            station_generation=StationGenerationConfig(
                ...
                min_distance_km=station_raw.get(
                    "min_distance_km", 0.3
                ),
