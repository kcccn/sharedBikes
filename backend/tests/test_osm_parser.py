...
class TestParseMaxspeed:
    def test_none_returns_30(self) -> None:
        assert _parse_maxspeed(None) == 30.0

    def test_numeric_string(self) -> None:
        assert _parse_maxspeed("50") == 50.0

    def test_kmh_suffix(self) -> None:
        assert _parse_maxspeed("60 km/h") == 60.0

    def test_kph_suffix(self) -> None:
        assert _parse_maxspeed("70 kph") == 70.0

    def test_mph_conversion(self) -> None:
        result = _parse_maxspeed("30 mph")
        assert math.isclose(result, 30 * 1.609344, rel_tol=1e-4)

    def test_list_takes_minimum(self) -> None:
        assert _parse_maxspeed(["30", "50", "80"]) == 30.0

    def test_list_with_mixed_formats(self) -> None:
        result = _parse_maxspeed(["30 mph", "50 km/h"])
        # 30 mph = 48.28 km/h, min should be 48.28
        expected = 30 * 1.609344
        assert math.isclose(result, expected, rel_tol=1e-4)

    def test_list_empty_returns_30(self) -> None:
        assert _parse_maxspeed([]) == 30.0

    def test_unrecognised_format_returns_30(self) -> None:
        assert _parse_maxspeed("free") == 30.0
...
class TestParseSingleMaxspeed:
    ...
    def test_with_kmh(self) -> None:
        assert _parse_single_maxspeed("50 km/h") == 50.0

    def test_with_kph(self) -> None:
        assert _parse_single_maxspeed("50 kph") == 50.0
    ...
    def test_decimal(self) -> None:
        assert _parse_single_maxspeed("30.5") == 30.5

    def test_unrecognised_returns_30(self) -> None:
    ...
    def test_float_value(self) -> None:
        assert _parse_single_maxspeed(45.5) == 45.5
