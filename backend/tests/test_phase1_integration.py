...
All tests use the synthetic Beijing grid (no real OSM data required).
"""
...
def test_load_beijing_city_structure() -> None:
    ...
    # Every station must have a valid position within Beijing range
...
def test_cache_hit_performance() -> None:
    """Cached city loading must be fast (< 2s). Cold load should be < 30s."""
    service = MapService()
    service.clear_cache()
